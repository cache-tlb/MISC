...
设 $f(q) = y'x - x'y + qx' = (1-q)f(0) + qf(1)$, 而 $f(0) > 0, f(1) > 0, 0\le q\le 1$, 所以 $f(q)$ 也是恒大于 $0$. 说明 $\tan\theta_q$ 也是随着 $t$ 增加而单调递增的. 进而可得当 $Q$ 位于 $AB$ 之间任意一点时, 从 $Q$ 看向 $y$ 轴右侧的半边曲线也不会发生遮挡.

对于 $y$ 轴左侧的半边曲线, $\theta_q$ 的范围是 $(\dfrac{\pi}{2}, \dfrac{3\pi}{2})$, 类似的也可以推出从 $Q$ 看向 $y$ 轴左侧的半边曲线也不会发生遮挡.
