...
考虑 $x$ 在 $(a,b)$ 区间上的加权平均, 其中 $a > 0$:
$$ f(t) = \frac{\int_a^b x\cdot w_t(x)\ \mathrm{d}x}{\int_a^b w_t(x)\ \mathrm{d}x} = \frac{\int_a^b x^{t+1}\ \mathrm{d}x}{\int_a^b x^{t}\ \mathrm{d}x} ,$$
这里权重函数就是 $w_t(x) = x^t$. 对于任意的 $x_2 > x_1$, 如果 $x_2$ 拥有更大的权重, 则加权平均结果就越大. 事实上, 随着 $t$ 增加, $(\dfrac{x_2}{x_1})^t$ 是增加的, 说明 $f(t)$ 是严格递增的函数.

算术平均数 
$$A(a,b) = f(0) = \frac{b^2-a^2}{a(b-a)} = \frac{a+b}{2} .$$

几何平均数
$$ G(a,b) = f(-\frac{3}{2}) = \frac{\sqrt{b}-\sqrt{a}}{\frac{1}{\sqrt{a}} - \frac{1}{\sqrt{b}} } = \sqrt{ab} .$$

调和平均数
$$ H(a,b) = f(-3) = \frac{\frac{1}{b}-\frac{1}{a}}{\frac{1}{2}(\frac{1}{b^2}-\frac{1}{a^2})} = \frac{2}{\frac{1}{b}+\frac{1}{a}}.$$

对数平均数
$$ L(a,b) = f(-1) = \frac{\ln b - \ln a}{b-a} .$$

中心平均数
$$ T(a,b) = f(1) = \frac{2(b^3-a^3)}{3(b^2-a^2)} = \frac{2(a^2+ab+b^2)}{3(a+b)} .$$

希罗平均数
$$ N(a,b) = f(-\frac{1}{2}) = \dfrac{\frac{2}{3}(b^\frac{3}{2} - a^\frac{3}{2})}{2(b^{\frac{1}{2}} - a^{\frac{1}{2}})} = \frac{a+\sqrt{ab}+b}{3} .$$
\newpage
%------------------------------------------------------------------------------%
...
从几何上来看, 对球面上的两个点 $\bold{n}$ 和 $\bold{n}'$, 只要它们足够接近, 则 $\bold{n}'$ 到 $\bold{n}$ 的方向 $\mathrm{d}\bold{n}$ 总是在球面上 $\bold{n}$ 点的切平面上, 因此与$\bold{n}$ 是垂直的.
...
曲率还刻画了曲线上的点远离切线的程度. 考察曲线上的点 $\bold{p}(t) = (x(t),y(t))$, 其附近一点为 $\bold{p}(t+\Delta t)=\bold{p}(t)+\Delta\bold{p}$, 进行二阶泰勒展开:
$$ \Delta{\bold{p}} = \dot{\bold{p}}\Delta t + \frac{1}{2}\ddot{\bold{p}}\Delta t^2 + o(\Delta t^2) $$
\begin{figure*}[htbp]
\centering
...
\coordinate[] (E) at (4,3);
\coordinate[] (F) at (5,3);
\coordinate[] (G) at (4.5,1.5);
\coordinate[] (M) at (3,0);
\draw [line width=1pt,-Stealth] (D) -- (C);
\draw [line width=1pt,-Stealth] (G) -- (E);
\draw [line width=1pt,red] (E) to[edge label=$\Delta l$](F);
\draw [line width=1pt] (E) -- (A);
\draw [line width=1pt] (M) -- (A);
\draw [line width=1pt,-Stealth] (B) -- (M);
\draw [line width=1pt] (B) -- (G);
\draw [line width=1pt,dashed] (A) -- (G);
\draw [line width=1pt,gray] (B) arc(0:60:5);
\draw [line width=1pt,gray] (B) arc(0:-30:5);
\draw[anchor=west] (C) node {$\dot{\bold{p}}$};
\draw[anchor=west] (B) node {$\bold{p}$};
\draw[anchor=east] (E) node {$\bold{p} + \Delta\bold{p}$};
\draw[anchor=north] (M) node {$\bold{n}$};
\draw pic["$\alpha$",draw,line width=1pt, angle eccentricity=1.8, angle radius=0.6cm] {angle=C--B--E};
\draw pic["$\alpha$",draw,line width=1pt, angle eccentricity=1.8, angle radius=0.6cm] {angle=B--A--G};
\foreach \p in {A,B}
	\fill[fill=black,draw=black,thick] (\p) circle (1.0pt);
\end{tikzpicture}
\end{figure*}

记 $\bold{p}(t)$ 点处切线为直线 $T$, 单位法向量为 $\displaystyle \bold{n} = \frac{(\dot{y}, -\dot{x})}{\sqrt{\dot{x}^2+\dot{y}^2}}$ , 则$\bold{p}(t+\Delta t)$ 到切线 $T$ 的距离 $\Delta l$ 就是 $\Delta\bold{p}$ 向量到 $\bold{n}$ 上的投影:
\begin{align*}
\Delta l &= \Delta\bold{p} \cdot \bold{n} = \frac{(-\dot{y},\dot{x})}{\sqrt{\dot{x}^2+\dot{y}^2}} \cdot \left( (\dot{x},\dot{y})\Delta t + \frac{1}{2}(\ddot{x},\ddot{y})\Delta t^2 + o(\Delta t^2)\right)\\
&= \frac{\ddot{y}\dot{x}-\ddot{x}\dot{y}}{2\sqrt{\dot{x}^2+\dot{y}^2}} \cdot \Delta t^2 + o(\Delta t^2)
\end{align*}
随着 $\Delta t$ 变化, $\Delta l$ 也是变化的.
从几何上看, 假设 $\bold{p}(t)$ 与 $\bold{p}(t+\Delta t)$ 位于半径为 $R$ 的圆周上, $T$ 为切线, $\Delta\bold{p}$ 为弦, 弦切角$\alpha$ 是弦所对圆心角的一半, 分别考虑弦切角和圆心角所在三角形, 满足:
$$
\frac{\Delta l}{|\Delta \bold{p}|} = \sin\alpha  = \frac{1}{2}\cdot\frac{|\Delta \bold{p}|}{R},
$$
故有
$$
\kappa = \frac{1}{R} = \lim_{\Delta t\to 0}\frac{2\Delta l}{| \Delta\bold{p} |^2} = \frac{\ddot{y}\dot{x}-\ddot{x}\dot{y}}{\sqrt{\dot{x}^2+\dot{y}^2}^3} 
$$
...
对于一个凸的 $n$ 边形, 设顶点为 $A_1, A_2, \cdots, A_n$. 假如多边形内部存在一点 $P$, 使每个顶点 $A_i$ 到 $P$ 延长线 $A_iP$ 恰好与各边有一个交点, 则 $n$ 是奇数.

或者等价的命题, 当 $n$ 为偶数时, 多边形内部任意一点 $O$, 总有两个顶点到 $O$ 的连线, 交于同一边上.

~

解: 证明等价命题. 取凸 $2n$ 边形任意的相对的两个点连线, 会将原来的多边形分成两个凸的 $n+1$ 边形. 设为 $X$ 和 $Y$. 点 $O$ 一定落在某一半 (不妨设为 $X$) 的内部或边界. 取另一半的 $n+1$ 个顶点分别与 $O$ 连线并延长, 每条线会先由 $XY$ 的公共边进入$X$, 再由 $X$ 的另一边(共 $n$ 种可能)离开. 由抽屉原理, 必然有两条线在同一条边上离开 $X$.
...
\coordinate[] (J) at (-3.517,0.522);
\coordinate[] (K) at (-3.774, 1.323);
\coordinate[] (L) at (-3.714, 2.571);
\coordinate[] (M) at (-2.75, -1.125);
\coordinate[] (N) at (-0.647, -1.882);
\draw[line width=1pt] (B) -- (C) -- (D) -- (E) -- (F);
\draw[line width=1pt,gray] (F) -- (G) -- (H) -- (A) -- (B);
\draw[line width=1pt,gray,dashed] (B)-- (F);
\draw[line width=1pt,cyan!30] (B)-- (N);  
\draw[line width=1pt,blue!30] (D)-- (J); 
\draw[line width=1pt,green!30] (E)-- (K); 
\draw[line width=1pt,orange!30] (F)-- (L); 
\draw[line width=1pt,magenta!30] (C)-- (M); 
\foreach \p in {A,B,C,D,E,F,G,H,I,J,K,L,M,N}
	\fill[fill=black,draw=black,thick] (\p) circle (1pt);
\draw[] (1,2) node {$Y$};
\draw[] (-1,0) node {$X$};
\draw[anchor=north] (I) node {$O$};
\end{tikzpicture}
\end{figure*}
