...
$$ \frac{x}{\cos\theta} + \frac{y}{\sin\theta} = 1 ,$$
或写成
$$ \tan\theta\cdot x + y = \sin\theta \qquad \cdots (1)$$
曲线上每一点的切线对应棍子的某一时刻的状态, 即:
$$
\frac{\mathrm{d}y}{\mathrm{d}x}= -\tan\theta \qquad \cdots (2)
$$
将它代入上面的式子得微分方程:
...
\begin{figure*}[htbp]
\centering
\begin{tikzpicture}[scale=10,domain=0:1]
\coordinate (O) at (0,0);
...
\begin{align*}
x &= v_0\cos\theta\cdot t \qquad\qquad\quad \cdots (1) \\
y &= v_0\sin\theta\cdot t - \frac{1}{2}gt^2 \qquad \cdots (2) 
\end{align*}
