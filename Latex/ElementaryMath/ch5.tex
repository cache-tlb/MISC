...
于是猜测 $a_n = 1 + \dfrac{1}{2} + \cdots + \dfrac{1}{n}$. 下面用归纳法证明. 前面分析对 $n=1,2,3$ 都成立. 假设对 $n = k (\ge 3)$ 成立, 对于 $n = k+1$, 将最快的那个点单独考虑, 余下 $k$ 个点. 若最快的点在最前面, 则不会被剩下的点追上, 这样的情况数共有 $k!$ 种, 剩余点数总数为 $(a_k+1)k!$. 
