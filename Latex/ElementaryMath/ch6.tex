...
\newpage
%------------------------------------------------------------------------------%
\noindent 任给一个图, 证明总可以给每个顶点染上黑色或白色, 使得每个白色(黑色)顶点的邻接的顶点中至少有一半是黑色(白色)的.

~

解: 设图的所有边总数为 $m$. 一条边的两个端点颜色相同则称为同色边, 否则称为异色边, 设这两种边的个数
分别是 $m_1$ 和 $m_2$, 则 $m_1, m_2\le m$ 且 $m_1+m_2=m$.

对于任意一种着色方案, 如果存在一个顶点(不妨称为$A$), 其多于一半的邻接节点与$A$的颜色相同, $A$关联的同色边和异色边数量分别为 $a_1, a_2$, 则有 $a_1 > a_2$. 此时将 $A$ 的颜色取反, 原来 $A$ 关联的同色边变成了异色边, 原来关联的异色边变成了同色边, 而其他的边属性不变. 于是异色边的总数变成 $m'_1 = m_1+a_1-a_2 > m_1$.

这说明, 只要存在这样的顶点 $A$, 就可以不断通过将它的颜色反过来, 使 $m_1$ 严格增加. 而 $m_1$ 的上限是 $m$, 因而 $m_1$ 有最大值. 达到最大值后, 就不会存在这样的顶点了.
